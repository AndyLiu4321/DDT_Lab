...
class MiniStairsEnvCfg(MiniRecoveryRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        stairs_cfg.step_height_range = (0.03, 0.3)  # 台阶高度随难度从 3 cm 增至 20 cm
        ...
        self.curriculum.terrain_levels = CurrTerm(
            ...
            params={
                "success_distance": 1.0,  # 向 +X 前进超过 1.0 m 时升级
                "failure_distance": 0.4,  # 向 +X 前进不足 0.4 m 时降级
            },
        )
        ...
        # ------------------------------------------------------------------ #
        # 初级训练关闭外力和周期推搡，避免策略用大量精力处理与爬楼无关的扰动。    #
        # 其余摩擦、质量和执行器随机化仍从父类继承，用于保持一定鲁棒性。          #
        # ------------------------------------------------------------------ #
